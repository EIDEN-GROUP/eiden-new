...
const CLIENT = "Anisal";
const YEAR = "2025";
const CATEGORY: Say = { fr: "Coopérative", en: "Cooperative" };
const LOCATION: Say = {
  fr: "Région de Souss-Massa, Maroc",
  en: "Souss-Massa, Morocco",
};
...
const IDENTITY = {
  ...
  signals: [
    { fr: "Les mêmes lignes, tournées.", en: "The same lines, turned." },
    {
      fr: "Une figure différente à chaque produit.",
      en: "A different figure for each product.",
    },
    { fr: "La même famille.", en: "One family." },
  ] as Say[],
};
...
const DIGITAL = {
  ...
  lines: [
    { fr: "Ce que la coopérative produit.", en: "What the cooperative makes." },
    { fr: "Comment.", en: "How." },
    { fr: "Par qui.", en: "By whom." },
  ] as Say[],
};
...
const IMPACT = {
  ...
  outcomes: [
    {
      fr: "Une gamme qui se reconnaît d'un produit à l'autre.",
      en: "A range recognisable from one product to the next.",
    },
    {
      fr: "Un emballage unique à la place d'autant de versions que de points de vente.",
      en: "A single packaging in place of one version per point of sale.",
    },
    {
      fr: "De quoi répondre à un acheteur qui n'est jamais venu.",
      en: "Something to answer a buyer who has never visited.",
    },
  ] as Say[],
};
...
const CHAPTERS = [
  ...
  { id: "identite", label: { fr: "Identité", en: "Identity" } },
  {
    ...
    label: { fr: "Présence digitale", en: "Digital presence" },
  },
  { id: "impact", label: { fr: "L'impact", en: "The impact" } },
];

export function AnisalV2View() {
  const say = useSay();
  ...
  return (
    ...
    >
      ...
      <Chapter id="identite">
        <Caption
          index={3}
          label={say({ fr: "Identité · Packaging", en: "Identity · Packaging" })}
          title={say(IDENTITY.title)}
          text={say(IDENTITY.text)}
        />
        <Pair>
          <IdentityPlate
            ground="#FFFFFF"
            image="/clients/anisal.png"
            alt={say({ fr: "Logo Anisal", en: "Anisal logo" })}
            contain
          />
          <SignalsPanel items={IDENTITY.signals.map(say)} />
        </Pair>
      </Chapter>

      <Chapter id="presence-digitale">
        <Caption index={4} title={say(DIGITAL.title)} text={say(DIGITAL.text)} />
        <LinesRow items={DIGITAL.lines.map(say)} tone="forest" />
      </Chapter>
