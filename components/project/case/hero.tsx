...
export function CaseHero({ project }: { project: ProjectCase }) {
  ...
  return (
    <section ref={ref} data-nav-tone="light" className="hero-depart relative isolate flex min-h-[68svh] flex-col overflow-hidden bg-beige sm:min-h-[74svh]">
      ...
      <div className="container-eiden relative flex flex-1 flex-col pt-28 pb-10 sm:pt-20 sm:pb-10">
        ...
        <div className="mt-10 pt-3">
          ...
          <p className={cn( ENTER, "editorial text-ink mt-8 max-w-[34ch] text-[clamp(1.375rem,3vw,2.125rem)]", )} style={stage(0.5)}>
            {say(project.hero.statement)}
          </p>

          {/* ④ WHAT. One short paragraph, never two. */}
          <p className={cn( ENTER, "text-ink/60 mt-6 max-w-[52ch] text-[0.9375rem] leading-relaxed sm:text-[1.0625rem]", )} style={stage(0.6)}>
          ...
          {/* <ul className={cn(ENTER, "mt-9 flex flex-wrap gap-2")} style={stage(0.7)}>
            {project.architecture.chain.map((item) => (
              <li key={say(item)} className="border-ink/20 text-ink/75 font-label rounded-full border px-4 py-1.5 text-[0.7rem] font-bold tracking-[0.16em] uppercase sm:text-[0.75rem]">
                {say(item)}
              </li>
            ))}
          </ul> */}
          ...
          {/* ⑥ Metadata. */}
          <p className={cn( ENTER, "border-ink/12 text-ink/45 mt-9 border-t pt-5 text-[0.8125rem] tracking-[0.02em]", )} style={stage(0.78)}>
            {rail}
          </p>
        </div>
      </div>
