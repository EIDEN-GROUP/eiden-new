  ...
  return (
    ...
    <section className="grain py-24 sm:py-32">
      <div className="container-eiden relative z-2">
        ...
        <ol ref={stepsRef} style={{ "--n": `${total}` } as CSSProperties} className="mt-14 grid gap-x-8 gap-y-12 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-12">
          {methods.map((method, index) => (
            <li key={method.name} style={{ "--i": `${index}` } as CSSProperties} className="step-in relative">
              <div className="flex items-center gap-4">
                ...
                {index < total - 1 ? (
                  <span aria-hidden className={cn( "step-arrow hidden flex-1 items-center", index % 2 === 1 ? "lg:flex" : "sm:flex", )}>
                    <span className="bg-teal/35 h-px flex-1" />
                    <ArrowRight className="text-teal/60 -ml-1 size-4 shrink-0" strokeWidth={1.5} />
                  </span>
                ) : null}
              </div>

              {index < total - 1 ? (
                <span aria-hidden className="step-run absolute inset-x-0 -bottom-12 flex h-12 flex-col items-center sm:hidden">
                  <span className="bg-teal/35 w-px flex-1" />
                  <ArrowDown className="text-teal/60 -mt-1 size-4 shrink-0" strokeWidth={1.5} />
                </span>
              ) : null}
