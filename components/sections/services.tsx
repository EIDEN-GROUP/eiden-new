  ...
  return (
    <section id="services" className="bg-canvas pt-20 sm:pt-28">
      <div className="container-eiden">
        ...
        <RevealWords as="h2" text={t.services.title} delay={0.05} className="text-ink mt-6 max-w-4xl text-[clamp(2rem,4.6vw,3.5rem)] leading-[1.02]" />
      ...
      <div className="mt-14 lg:mt-16">
        {t.services.items.map((item, index) => {
          ...
          return (
            <Link key={item.slug} href="/nos-solutions" className={cn( "group border-ink/10 relative isolate block border-t", index === t.services.items.length - 1 && "border-b", )} >
              {media ? (
                <span aria-hidden className={cn( "absolute inset-0 -z-10 overflow-hidden transition-opacity duration-700 ease-[var(--ease-brand)] motion-reduce:transition-none", "opacity-100 lg:opacity-0 lg:group-hover:opacity-100", )}>
                  <Image src={media} alt="" fill sizes="100vw" className="object-cover object-center" />
