...
const accents = ["text-mint", "text-gold", "text-mint", "text-gold"];
...
export function Offer() {
  ...
  return (
    ...
    >
      ...
      >
        <div className="border-mint/40 absolute top-1/4 -right-24 size-[28rem] rounded-full border" />
        <div className="border-gold/30 absolute top-1/3 -right-10 size-[20rem] rotate-45 border" />
        <div className="absolute bottom-10 -left-32 size-[26rem] rounded-full bg-[radial-gradient(circle,rgba(12,87,82,0.7),transparent_70%)]" />
      </div>

      <div className="container-eiden relative z-[2]">
        <Reveal direction="none" duration={0.5}>
          <p className="eyebrow text-mint">{t.offer.eyebrow}</p>
        </Reveal>

        <div className="mt-8 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <Reveal delay={0.05}>
            <h2 className="text-canvas max-w-2xl text-[clamp(1.75rem,4vw,3rem)]">
              {t.offer.title}
            </h2>
          </Reveal>
          ...
          {t.offer.items.map((item, index) => {
            ...
            return (
              <article
                ...
                className={cn(
                  "group border-canvas/12 relative border-t transition-colors duration-500 ease-[var(--ease-brand)]",
                  index === t.offer.items.length - 1 && "border-b",
                  isActive ? "bg-canvas/[0.055]" : "bg-transparent",
                )}
              >
                <div className="grid gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[auto_minmax(0,1.05fr)_minmax(0,1fr)] lg:items-start lg:gap-10 lg:py-10">
                  ...
                  <h3 className="font-display text-canvas text-[clamp(1.5rem,3.4vw,2.75rem)] leading-[0.98] font-extrabold tracking-[-0.04em] uppercase">
                    {item.title}
                  </h3>
